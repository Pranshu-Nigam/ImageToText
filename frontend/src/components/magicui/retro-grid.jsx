import { cn } from "@/lib/utils";

export default function RetroGrid({ className, angle = 65 }) {
  return (
    <div
      className={cn(
        "pointer-events-none absolute h-full w-full overflow-hidden [perspective:200px] opacity-50",
        className,
      )}
      style={{ "--grid-angle": `${angle}deg` }}
    >
      {/* Grid */}
      <div className="absolute inset-0 [transform:rotateX(var(--grid-angle))]">
        <div
          className={cn(
            "animate-grid",
            "[background-repeat:repeat] [background-size:60px_60px] [height:300vh] [inset:0%_0px] [margin-left:-50%] [transform-origin:100%_0_0] [width:600vw]",
            // Dynamic line colors mapped to light and dark themes
            "[background-image:linear-gradient(to_right,var(--retro-grid-line)_2px,transparent_0),linear-gradient(to_bottom,var(--retro-grid-line)_2px,transparent_0)]",
          )}
        />
      </div>

      {/* Gradient Mask */}
      <div className="absolute inset-0 bg-gradient-to-t from-background to-transparent to-90%" />
    </div>
  );
}
